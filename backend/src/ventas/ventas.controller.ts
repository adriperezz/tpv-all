...
export class VentasController {
  ...
  @Get()
  @Roles(Rol.ADMIN)
  @ApiOperation({ summary: 'Ventas por rango de fechas (admin)' })
  @ApiQuery({ name: 'desde', required: false, description: 'ISO datetime' })
  @ApiQuery({ name: 'hasta', required: false, description: 'ISO datetime' })
  findByPeriodo(@Query('desde') desde?: string, @Query('hasta') hasta?: string) {
    return this.service.findByPeriodo(desde, hasta);
  }
