...
export class DashboardService {
  ...
  async resumenPeriodo(desdeStr?: string, hastaStr?: string) {
    const inicio = desdeStr ? new Date(desdeStr) : new Date(0);
    const fin = hastaStr ? new Date(hastaStr) : new Date();
    return this.buildResumen(inicio, fin);
  }

  async resumenDia(fecha?: string) {
    const { inicio, fin } = rangoFecha(fecha);
    const r = await this.buildResumen(inicio, fin);
    return { ...r, fecha: fecha ?? new Date().toISOString().slice(0, 10) };
  }

  private async buildResumen(inicio: Date, fin: Date) {
