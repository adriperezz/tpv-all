...
export class VentasService {
  ...
  async cobrar(dto: CobrarVentaDto, usuarioId: number) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const linea of dto.lineas) {
        ...
        await tx.lineaVenta.create({
          data: {
            ...
            subtotal,
            ...(linea.comentario ? { comentario: linea.comentario } : {}),
          },
        });

        // Genera tickets físicos por tipo — un solo update por tipo (atómico)
        for (const pt of producto.productoTickets) {
          const totalTickets = pt.cantidad * linea.cantidad;

          // Incrementa el contador de golpe y obtiene el valor final
          const tipoActualizado = await tx.tipoTicket.update({
            where: { id: pt.tipoTicketId },
            data: { contadorSerie: { increment: totalTickets } },
          });

          // Los números de serie van desde (final - totalTickets + 1) hasta final
          const serieBase = tipoActualizado.contadorSerie - totalTickets + 1;
          await tx.ticketFisico.createMany({
            data: Array.from({ length: totalTickets }, (_, i) => ({
              tipoTicketId: pt.tipoTicketId,
              numeroSerie: serieBase + i,
              ventaId: venta.id,
      ...
      if (resultado && resultado.tickets.length > 0) {
        ...
        const tickets = this.printer.buildTickets(tipos);
        this.printer.dispararImpresion(dto.impresoraIp, tickets);
      }
  ...
  async findByPeriodo(desdeStr?: string, hastaStr?: string) {
    const inicio = desdeStr ? new Date(desdeStr) : new Date(0);
    const fin = hastaStr ? new Date(hastaStr) : new Date();
